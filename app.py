...
def precipitation():
    ...
    results= session.query(Measurements.date, Measurements.prcp).filter(Measurements.date >= prev_last_date).order_by(Measurements.date.desc()).all()
...
def tobs():
     ...
     queryresult = session.query( Measurements.date, Measurements.tobs).filter(Measurements.station=='USC00519281')\
